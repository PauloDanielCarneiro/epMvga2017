...
void baricentrico(double& b1, double& b2, double& b3, double& xp, double& yp, int i){
    ...
    for (aux = 0; aux <= 2; aux++){
     ...
     double ABC = (1/4) * sqrt((AB + BP + AP)*(-AB + BP + AP)*(AB - BP + AP)*(AB + BP - AP)) 
     
     //Triangulo APC
     double CP = sqrt(pow(abs(coord[4] - xp), 2) + pow(abs(coord[5] - yp), 2));
     double semiAPC = (AC + CP + AP) / 2.0;
     double ABC = (1/4) * sqrt((AP + CP + AC)*(-AP + CP + AC)*(AP - CP + AC)*(AP + CP - AC)) 
     
 
     //Triangulo PBC
     double semiPBC = (CP + BP + BC) / 2.0;
     double ABC = (1/4) * sqrt((BP + BC + CP)*(-BP + BC + CP)*(BP - BC + CP)*(BP + BC - CP)) 
